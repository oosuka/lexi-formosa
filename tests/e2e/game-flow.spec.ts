...
test('開始前の level selector は高さが揃う', async ({ page }) => {
  await installMockWordlists(page);

  await page.goto('/');
  await expect(page).toHaveURL(/\/lexi-formosa\/$/);

  const detailCards = page.locator('.session-start-level-option');
...
test('game over 後に restart と reset の導線を使える', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Start Session' })).toBeVisible();
  await expect(page.locator('.session-start-panel')).toBeVisible();
});
