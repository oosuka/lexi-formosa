...
- 初回表示とリセット後は `Start Session` からセッション開始
- PC 向けの `Premium Launch Pad` レイアウト
- プレイ中は quiz panel 上部に `Score / Streak / Miss` を常時表示
- 開始前は level selector と 1 つの開始 CTA に集中し、終了時は結果サマリーとして情報を整理
- 正解 / 不正解をカード状態と結果帯で見分けやすく表示
- 音を消していても、正解 / 不正解が色・動き・カード状態で見分けやすい
- キーボードでは `1-4` で回答し、`Enter` で開始や次の問題へ進行可能
- 開始画面では選択中レベルの最高 `Score / Streak` を表示し、`localStorage` に保存
