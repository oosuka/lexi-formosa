...
- 日付・レベル・当日の完走数から端末内で再現できる「今日の10語」ルート。完走後は未出題中心の次の10語へ進み、日付をまたいだ後の新しいルート開始時は当日の記録へ切り替えます。外部 DB やログインは不要
...
- 390px 前後のスマホ幅を中心に、開始ボタン、10問の進行、スコア、連続数、連続不正解で終了までの回数、4択を追いやすく設計。Level 3 の長い単語は狭い幅で折り返し、横スクロールを発生させません。PC 幅にもレスポンシブ対応
...
テストは、出題ロジック、状態遷移、語彙検証、非同期失敗時の継続、主要なゲーム導線を優先します。CSS の細部や重複した表示構造だけを固定するテストは避けます。現在のデザイン方針として、`tests/unit/design-system.test.ts` でグラデーション背景とフルピル形状の再導入を防ぎ、Playwright で狭い幅の Level 3 長語が横にはみ出さないことを確認しています。
