...
- PC 版の最高記録カードはクリックでレベル選択と連動し、スマホ版は選択中レベルの `Level / words` と最高 `Score / Streak` を省スペース表示
- 不正解時は `正解は「xxx」です。残りn回で終了します。` の形で正解と残り回数を表示
- `Life` が 1 の状態で次の問題へ進んだ直後だけ、単語カード全体を 3 段の弱い pulse で知らせる。`prefers-reduced-motion` では無効
