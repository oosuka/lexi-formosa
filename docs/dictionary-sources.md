...
- `data/vocabulary-candidates.json` はレビュー用の中間成果物で、`npm run review:vocab:export -- --limit=500`、`npm run review:vocab:export -- --level=3 --risk-only --limit=200`、`npm run review:vocab:export -- --level=3 --limit=20` の入力になります。
...
Level 1-2 通常レビューが空になった後は、`--level=3 --limit=20` で Level 3 low-risk Challenge 候補を少量ずつ確認します。
`--limit=500` は技術上の上限ではありません。`1000` 件以上も指定できますが、誤訳や reject 対象を安全に確認する単位としては Level 3 high-risk は 200 件、Level 1-2 通常レビューは 500 件、Level 3 low-risk は 20 件を推奨します。
