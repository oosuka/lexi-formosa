...
- 最新作業: `2026-04-17 Level 3 low-risk Challenge batch 50 件を処理（承認 41 件、却下 9 件）`
- 公開語彙数: `15121`
- `data/editorial-overrides.json`: `12619` 件
- 内訳: `approved 12004`、`rejected 615`
- Level 1: `10744` 件、reviewed `10699`、unreviewed `45`
- Level 2: `1115` 件、reviewed `1075`、unreviewed `40`
- Level 1-2 の unreviewed `85` 件はすべて `manual-vocabulary` 由来の seed 語彙。通常 export queue には出さない curated seed として扱う。
- Level 3: `3262` 件、reviewed `230`、unreviewed `3032`
- Level 3 内訳: 5文字 `2319`、6文字 `943`
- Level 3 MJdic 単独: `3243`
...
- 次の推奨 batch: `npm run review:vocab:export -- --level=3 --limit=50`
...
Level 1-2 の通常 export queue は `0` になった。残 unreviewed `85` 件はすべて curated seed で、通常 export の対象外。
2026-04-17 の Level 3 low-risk Challenge batch `20` 件を処理済み。承認 `19` 件、却下 `1` 件。次に進めるのは Level 3 low-risk Challenge Review の継続。
2026-04-17 の追加 Level 3 low-risk Challenge batch `50` 件を処理済み。承認 `41` 件、却下 `9` 件。次回以降の low-risk review は `50` 件単位で継続する。
...
4. Level 1-2 通常 batch が `0` なら、Level 3 low-risk batch を 50 件単位で処理
5. 再生成・検証
6. この plan の Current State を更新
